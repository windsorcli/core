...
| <a name="input_dns_servers"></a> [dns\_servers](#input\_dns\_servers) | Nameservers for container /etc/resolv.conf (e.g. terraform\_output(workstation, dns\_internal\_ip)). Applies to every container this module creates (cluster\_nodes and instances alike). When unset, containers use the Docker daemon's default embedded resolver. | `list(string)` | `null` | no |
