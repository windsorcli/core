...
| <a name="input_availability_zones"></a> [availability\_zones](#input\_availability\_zones) | Zones node pools spread across. Null (cheap single-zone default) lets AKS place nodes without a zone constraint; set ['1','2','3'] under topology: ha for multi-zone spread. A pool's own availability\_zones overrides this. | `list(string)` | `null` | no |
