...
### Upgrading: retiring the `autoscaled` node pool

The standalone `autoscaled` user pool has been folded into the portable `pools` model. When `var.pools` is empty, the module now materializes a single autoscaling `general` pool (Standard_D4s_v3, min 1 / max 3) in its place, so a zero-config deploy keeps the same workload capacity. The inline default pool is unchanged and stays the system pool.

Because AKS node pool names are immutable, this is a destroy-and-create, not an in-place rename: `terraform apply` destroys `autoscaled` and creates `general` in the same plan, and workloads on the old pool are drained and rescheduled. There is no state-move that avoids it. On a single-pool cluster, schedule the apply in a maintenance window, or pre-create a replacement via `var.pools` and migrate workloads before applying. Declaring any `cluster.pools` entry suppresses the fallback, so operators who already set `pools` are unaffected.
...
| [azurerm_federated_identity_credential.openbao](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/federated_identity_credential) | resource |
| [azurerm_key_vault.key_vault](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/key_vault) | resource |
| [azurerm_key_vault.openbao](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/key_vault) | resource |
...
| [azurerm_key_vault_key.openbao_unseal](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/key_vault_key) | resource |
...
| [azurerm_role_assignment.external_dns_private_zones](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
| [azurerm_role_assignment.external_dns_zones](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
...
| [azurerm_role_assignment.openbao_key_vault_officer](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
| [azurerm_role_assignment.openbao_key_vault_user](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment) | resource |
...
| [azurerm_user_assigned_identity.openbao](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/user_assigned_identity) | resource |
...
| <a name="input_availability_zones"></a> [availability\_zones](#input\_availability\_zones) | Availability zones every node pool is placed in. Null places nodes without a zone constraint; a list (e.g. ["1","2","3"]) spreads them across those zones. | `list(string)` | `null` | no |
...
| <a name="input_cert_manager_dns_zone_ids"></a> [cert\_manager\_dns\_zone\_ids](#input\_cert\_manager\_dns\_zone\_ids) | Full Azure resource IDs of DNS zones cert-manager is allowed to write ACME challenge records to. The DNS Zone Contributor role assignment is scoped to these zones — leave empty when create\_cert\_manager\_identity is false. | `list(string)` | `[]` | no |
| <a name="input_class_instance_types"></a> [class\_instance\_types](#input\_class\_instance\_types) | Default Azure VM size list per portable pool class. AKS pools accept a single SKU per pool — only the first entry is used; remaining entries document an operator preference order rather than an AKS-enforced fallback (the AWS-EKS equivalent does take a list at the API level). A pool's explicit instance\_types overrides this map. When overriding this variable, all seven class keys must be supplied — partial overrides are rejected at validate time. | `map(list(string))` | <pre>{<br/>  "arm64": [<br/>    "Standard_D2pds_v5",<br/>    "Standard_D4pds_v5",<br/>    "Standard_E4pds_v5"<br/>  ],<br/>  "compute": [<br/>    "Standard_F4s_v2",<br/>    "Standard_F8s_v2",<br/>    "Standard_F16s_v2"<br/>  ],<br/>  "general": [<br/>    "Standard_D4s_v3",<br/>    "Standard_D4as_v3",<br/>    "Standard_D8s_v3",<br/>    "Standard_D8as_v3"<br/>  ],<br/>  "gpu": [<br/>    "Standard_NC4as_T4_v3",<br/>    "Standard_NC8as_T4_v3"<br/>  ],<br/>  "memory": [<br/>    "Standard_E4s_v3",<br/>    "Standard_E4as_v3",<br/>    "Standard_E8s_v3"<br/>  ],<br/>  "storage": [<br/>    "Standard_L8s_v3",<br/>    "Standard_L16s_v3"<br/>  ],<br/>  "system": [<br/>    "Standard_D2s_v3",<br/>    "Standard_D2as_v3",<br/>    "Standard_D4s_v3",<br/>    "Standard_D4as_v3"<br/>  ]<br/>}</pre> | no |
...
| <a name="input_create_cert_manager_identity"></a> [create\_cert\_manager\_identity](#input\_create\_cert\_manager\_identity) | Whether to provision a User-Assigned Managed Identity, DNS Zone Contributor role assignments, and Federated Identity Credential for cert-manager's azureDNS ACME DNS-01 solver. Enable when cert-manager will issue ACME certificates against an Azure DNS zone. | `bool` | `false` | no |
| <a name="input_create_external_dns_identity"></a> [create\_external\_dns\_identity](#input\_create\_external\_dns\_identity) | Whether to provision a User-Assigned Managed Identity, DNS Zone Contributor role assignments, and Federated Identity Credential for external-dns. Enable when external-dns will publish records to an Azure DNS zone. | `bool` | `true` | no |
| <a name="input_create_openbao_identity"></a> [create\_openbao\_identity](#input\_create\_openbao\_identity) | Whether to provision a dedicated Key Vault and key, a User-Assigned Managed Identity, Key Vault role assignments, and a Federated Identity Credential for OpenBao's Azure Key Vault auto-unseal. Enable when the secrets\_store addon runs OpenBao with the openbao driver. | `bool` | `false` | no |
| <a name="input_default_node_pool"></a> [default\_node\_pool](#input\_default\_node\_pool) | Configuration for the default node pool | <pre>object({<br/>    name                         = string<br/>    vm_size                      = string<br/>    os_disk_type                 = string<br/>    max_pods                     = number<br/>    host_encryption_enabled      = bool<br/>    min_count                    = number<br/>    max_count                    = number<br/>    node_count                   = number<br/>    only_critical_addons_enabled = bool<br/>    upgrade_settings = optional(object({<br/>      drain_timeout_in_minutes      = number<br/>      max_surge                     = string<br/>      node_soak_duration_in_minutes = number<br/>    }))<br/>  })</pre> | <pre>{<br/>  "host_encryption_enabled": true,<br/>  "max_count": 3,<br/>  "max_pods": 48,<br/>  "min_count": 1,<br/>  "name": "system",<br/>  "node_count": 1,<br/>  "only_critical_addons_enabled": true,<br/>  "os_disk_type": "Managed",<br/>  "upgrade_settings": {<br/>    "drain_timeout_in_minutes": 30,<br/>    "max_surge": "10%",<br/>    "node_soak_duration_in_minutes": 10<br/>  },<br/>  "vm_size": "Standard_D2s_v3"<br/>}</pre> | no |
...
| <a name="input_external_dns_dns_zone_ids"></a> [external\_dns\_dns\_zone\_ids](#input\_external\_dns\_dns\_zone\_ids) | Full Azure resource IDs of DNS zones external-dns is allowed to manage records in. The DNS Zone Contributor role assignment is scoped to these zones — leave empty when create\_external\_dns\_identity is false. | `list(string)` | `[]` | no |
...
| <a name="input_private_subnet_ids"></a> [private\_subnet\_ids](#input\_private\_subnet\_ids) | Private subnet IDs the AKS node pools attach to. The inline default (system) pool uses the first; user pools use the last. Pipe network/azure-vnet's private\_subnet\_ids output. | `list(string)` | `null` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_cert_manager_client_id"></a> [cert\_manager\_client\_id](#output\_cert\_manager\_client\_id) | Client ID of the cert-manager User-Assigned Managed Identity. Annotate the cert-manager ServiceAccount with azure.workload.identity/client-id=<this> so token exchange targets the right identity. Null when the identity isn't provisioned. |
| <a name="output_cert_manager_principal_id"></a> [cert\_manager\_principal\_id](#output\_cert\_manager\_principal\_id) | Principal (object) ID of the cert-manager UAMI — useful for ad-hoc role grants outside this module. |
| <a name="output_cluster_name"></a> [cluster\_name](#output\_cluster\_name) | Name of the AKS cluster. Consumed by kustomize substitutions (txt-owner-id, etc.). |
| <a name="output_cluster_oidc_issuer_url"></a> [cluster\_oidc\_issuer\_url](#output\_cluster\_oidc\_issuer\_url) | OIDC issuer URL of the cluster. Used to bind external Workload Identity Federation credentials to the cluster. |
| <a name="output_external_dns_client_id"></a> [external\_dns\_client\_id](#output\_external\_dns\_client\_id) | Client ID of the external-dns User-Assigned Managed Identity. Annotate the external-dns ServiceAccount with azure.workload.identity/client-id=<this> so token exchange targets the right identity. Null when the identity isn't provisioned. |
| <a name="output_external_dns_principal_id"></a> [external\_dns\_principal\_id](#output\_external\_dns\_principal\_id) | Principal (object) ID of the external-dns UAMI — useful for ad-hoc role grants outside this module. |
| <a name="output_openbao_client_id"></a> [openbao\_client\_id](#output\_openbao\_client\_id) | Client ID of the OpenBao User-Assigned Managed Identity. Annotate the OpenBao ServiceAccount with azure.workload.identity/client-id=<this> so token exchange targets the right identity. Null when the identity isn't provisioned. |
| <a name="output_openbao_key_name"></a> [openbao\_key\_name](#output\_openbao\_key\_name) | Name of the key OpenBao's Azure Key Vault seal wraps/unwraps with. Null when the key isn't provisioned. |
| <a name="output_openbao_key_vault_name"></a> [openbao\_key\_vault\_name](#output\_openbao\_key\_vault\_name) | Name of the Key Vault holding OpenBao's auto-unseal key. Null when the vault isn't provisioned. |
...
| <a name="output_subscription_id"></a> [subscription\_id](#output\_subscription\_id) | Subscription the cluster lives in. Required by cert-manager (azureDNS solver) and external-dns (Azure provider) when authenticating via Workload Identity. |
| <a name="output_tenant_id"></a> [tenant\_id](#output\_tenant\_id) | Azure AD tenant the cluster lives in. Required by cert-manager (azureDNS solver) and external-dns (Azure provider) when authenticating via Workload Identity. |
<!-- END_TF_DOCS -->
