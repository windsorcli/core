...
| <a name="input_system_node_pool"></a> [system\_node\_pool](#input\_system\_node\_pool) | Configuration for the system node pool. Fields default independently, so a caller can override just node\_count. | <pre>object({<br/>    machine_type        = optional(string, "n2-standard-2")<br/>    disk_size_gb        = optional(number, 50)<br/>    node_count          = optional(number, 1)<br/>    autoscaling_enabled = optional(bool, false)<br/>    min_count           = optional(number, 1)<br/>    max_count           = optional(number, 3)<br/>  })</pre> | `{}` | no |
