...
# Machine Secrets
#-----------------------------------------------------------------------------------------------------------------------
# Cluster identity (CA, etcd CA, k8s CA, bootstrap token, encryption secret) is
# generated by ./modules/secrets when no upstream secrets are supplied — the
# default for incus/metal/docker/aws/azure paths. The same submodule is also
# called by cluster/talos/config (via ../talos/modules/secrets) on the hyperv
# CIDATA path; in that case its outputs flow back into cluster/talos via
# var.machine_secrets / var.client_configuration and the count-gated invocation
# below is skipped. local.machine_secrets / local.client_configuration pick
...
# Endpoint precondition lives separately because moduule blocks don't accept
# a lifecycle block. terraform_data is the standard "vehicle for preconditions"
# pattern used elsewhere in this repo (e.g. compute/incus).
...
locals {
  ...
  # locally-invoked ./modules/secrets submodule (default). Every downstream
  # reference uses these locals — never the submodule output directly.
  machine_secrets      = var.machine_secrets != null ? var.machine_secrets : module.secrets[0].machine_secrets
  client_configuration = var.client_configuration != null ? var.client_configuration : module.secrets[0].client_configuration
