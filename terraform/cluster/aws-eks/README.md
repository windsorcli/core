...
| <a name="input_enable_cloudwatch_logs"></a> [enable\_cloudwatch\_logs](#input\_enable\_cloudwatch\_logs) | Whether to enable EKS control plane logging to CloudWatch. EKS owns the log group via its service role; the module doesn't create or manage it. | `bool` | `true` | no |
