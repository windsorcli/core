...
| <a name="input_enable_cloudwatch_logs"></a> [enable\_cloudwatch\_logs](#input\_enable\_cloudwatch\_logs) | Whether to enable EKS control plane logging to CloudWatch. EKS owns the log group via its service role; the module does not create or manage it, so there is no module-controlled retention or CMK encryption on the group. Ship logs to S3 (subscription filter / Firehose) if either is required. | `bool` | `true` | no |
