...
| <a name="input_preserve_logs_on_destroy"></a> [preserve\_logs\_on\_destroy](#input\_preserve\_logs\_on\_destroy) | When true, the EKS log group survives terraform destroy via skip\_destroy and the post-destroy mop-up is suppressed. Recreating a cluster with the same name will fail with ResourceAlreadyExistsException unless the orphan group is imported or deleted first. | `bool` | `true` | no |
