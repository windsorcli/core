...
| <a name="input_node_subnet_ids"></a> [node\_subnet\_ids](#input\_node\_subnet\_ids) | Private subnet IDs node groups launch into. Defaults to private\_subnet\_ids (all AZs). Pass a single-AZ subset for the cheap single-zone default; pass all for multi-AZ node spread. The control plane always uses private\_subnet\_ids. | `list(string)` | `null` | no |
