...
| <a name="input_preserve_logs_on_destroy"></a> [preserve\_logs\_on\_destroy](#input\_preserve\_logs\_on\_destroy) | When true, the EKS log group survives terraform destroy via skip\_destroy. False (default) deletes it; AWS may briefly recreate it during cluster shutdown writes, leaving a small orphan that is the accepted cost of opting out of preservation. Recreating a cluster with the same name after a preserve+destroy fails with ResourceAlreadyExistsException unless the orphan group is imported or deleted first. | `bool` | `false` | no |
