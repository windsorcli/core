...
READY=$(kubectl get databaseinstance.sql.gcp.upbound.io "${pg_instance_name}" \
  -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "")
if [ "$READY" != "True" ]; then
  echo "${pg_instance_name} DatabaseInstance not Ready yet, skipping this tick."
  touch /shared/skip
  exit 0
fi

kubectl get databaseinstance.sql.gcp.upbound.io "${pg_instance_name}" \
  -o jsonpath='{.status.atProvider.privateIpAddress}' > /shared/address

kubectl get secret "${pg_instance_name}"-admin-credentials \
  -n system-provisioning -o jsonpath='{.data.username}' | base64 -d > /shared/admin-username
kubectl get secret "${pg_instance_name}"-admin-credentials \
