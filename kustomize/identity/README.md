...
authenticates against the platform realm — no per-app flag. Core registers the OIDC
client, a Job generates the client secret, Kyverno replicates it to Grafana, and
Grafana maps the `platform-admins` group to the Grafana Admin role:
...
Supply the client secret from a store instead of generating it, opt out, or override
the role mapping:
...
- **Client secrets.** SSO client secrets never land in git. The source of truth lives
  in `system-identity` — supplied from a store (`grafana.client_secret: ${secret(...)}`)
  or generated by a one-shot Job — and Kyverno replicates it into each consuming
  namespace (which grants the Kyverno background controller cluster-wide secret access
  via an aggregated ClusterRole). Consumer pods use `optional: false` and wait for the
  Secret rather than start misconfigured.
...
| `keycloak/grafana` | identity + Grafana both enabled (`grafana.sso != false`) | `KeycloakOIDCClient` registering Grafana in the platform realm (client id `grafana`; v2alpha1 derives it from the resource name). Reads the `grafana-oidc-client` Secret in `system-identity`. |
| `keycloak/grafana-secret` | SSO inferred on and `grafana.client_secret` unset | One-shot Job that generates the `grafana-oidc-client` Secret in `system-identity` when the blueprint did not supply one. Idempotent; leaves an existing secret untouched. |
| `keycloak/grafana-sync` | identity + Grafana both enabled (`grafana.sso != false`) | Kyverno `ClusterPolicy` (plus a background-controller RBAC grant) that replicates the `grafana-oidc-client` Secret from `system-identity` into `system-observability` and keeps it in sync. |
