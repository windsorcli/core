...
| `crossplane/postgres/monitoring-exporter` | a chart opts in — `telemetry.metrics.enabled` (default true) gates it in `kustomize/demo`'s own facet entry | A `postgres_exporter` Deployment/Service/PodMonitor in `system-database`, driver-agnostic, reading the Secret a `DatabaseCredentials` resource with `roleName: <pg_instance_name>-monitor` publishes (see `kustomize/provisioning`'s `crossplane/database-credentials`). Requires `pg_instance_name`. `PodMonitor` (`monitoring.coreos.com/v1`) only exists once the metrics pipeline vendors prometheus-operator's CRDs. |
