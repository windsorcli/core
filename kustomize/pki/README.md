...
| `private_ca_key` | `private-issuer/ca` is enabled | PEM-encoded CA private key. Sourced from `terraform_output('pki', 'key')`. |
...
| `private-issuer/ca` | `pki.enabled: true` | Full private CA: the CA cert/key (generated or BYO, from `terraform_output('pki', ...)`) are rendered straight into the `private-ca-cert` Secret cert-manager's `ca`-type `ClusterIssuer` (`private`) signs against, and the `private-ca-cert` ConfigMap trust-manager's `Bundle` distributes to workload namespaces, plus a Kyverno mutation policy that injects the trust bundle into workload Pods. |
