...
| `private-issuer/ca` | `pki.enabled: true` | Full private CA: the CA cert/key (generated or BYO, from `terraform_output('pki', ...)`) populate the `private-ca-keypair` Secret cert-manager's `ca`-type `ClusterIssuer` (`private`) signs against, placed in both `system-pki` and `system-pki-trust` via the facet's `secrets:` block (never rendered in the Kustomization spec, and never through a Flux substitution — a substituted multi-line PEM value loses its line breaks). trust-manager's `Bundle` reads the cert straight from that Secret and distributes it to workload namespaces; a Kyverno mutation policy injects the trust bundle into workload Pods. |
