...
| Component | Enable when | Effect |
|---|---|---|
| `private-issuer/selfsigned` | `pki.enabled` unset (metal, docker, incus, vsphere, hyperv), or `gateway.access == 'private'` (aws, azure, hetzner) | ClusterIssuer `private` with `selfSigned: {}`. Issues the private gateway certificate when no private CA is configured; browsers warn until the CA cert is added to a trust store. |
