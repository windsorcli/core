...
| `database/rds` | `demo.resources.database: true` AND `database.postgres.driver == 'rds'` | An `rds.aws.upbound.io/v1beta3` `Instance` CR `demo-db` (db.t4g.micro, 20Gi, AWS-managed master password, network-scoped to the cluster's own security group) — just the database definition, no provider wiring, no RBAC, no CronJob of its own. The application credential (`demo_app`, least-privilege CRUD, via `crossplane/aws-rds/app-role`) and monitoring (`demo_monitor`/`postgres_exporter`, via `crossplane/aws-rds/postgres-exporter`) are both `kustomize/provisioning`'s reusable components instead, wired in by this facet's own `provisioning` entry — see ADR-0009 §7, §8. |
