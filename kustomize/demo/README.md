...
| `database/cloudsql` | `demo.resources.database: true` AND `database.postgres.cloud.enabled: true` AND `database.postgres.cloud.driver == 'cloudsql'` | A `sql.gcp.upbound.io/v1beta2` `DatabaseInstance` CR `demo-db` (db-f1-micro, private IP only) plus a `Database` CR `demo` — just the database definition, no provider wiring. `kustomize/provisioning`'s `crossplane/gcp-cloudsql` component's `WatchOperation` reacts to this CR directly, generating the admin password, applying it onto the instance's own admin `User`, and provisioning the connection config and monitoring role without this facet's involvement. |
