#!/usr/bin/env bash
# Unit-tests each Composition's embedded Python function via its sibling
# composition.test.py. Extracts .spec.pipeline[0].input.script to a real
# .py file so the test can import it as a module.
set -euo pipefail
